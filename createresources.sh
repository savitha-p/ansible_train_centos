...
openstack network create --share --external --enable --provider-network-type flat --provider-physical-network flat public_network
openstack subnet create --gateway 172.172.3.1 --ip-version 4 --network public_network --allocation-pool start=172.172.3.246,end=172.172.3.248 --no-dhcp --subnet-range 172.172.3.1/24 public_subnet
...
#Create cloudrc file on ansible node at /root directory
...
export OS_VERIFY='false'" >> /root/cloudrc
